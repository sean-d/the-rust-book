...
impl Config {
    // constructor that takes the passed in args iterator. we make it mutated since we will be iterating over the iterator
    // returns a Result<Config, &str> where the Config is OK and Err is the error message as &str

    // with args being an own type and we are returning a string slice, a lifetime needs to be setup.
    // we use static: error is a string literal that will live for the duration of the program...hence static.
    pub fn new(mut args: env::Args) -> Result<Config, &'static str> {
        // since the program name is args[0], we start at 1 via args.next()
        args.next();
        ...
        // grab query from args[1]
        let query = match args.next() {
            Some(arg) => arg,
            None => return Err("Didn't get a query string"),
        };

        // grab filename from args[2]
...
pub fn search<'a>(query: &str, contents: &'a str) -> Vec<&'a str> {
    ...
    */
    ...
    // this is a fancy way of saying: filter to only the lines that contain our query string
    // collect turns the iterator into a collection and that is returned
    // rust knows what collection to use as it's define in the return type above.
    contents
        .lines()
